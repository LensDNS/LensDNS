...
**LensDNS** 是基于 [AdGuard Home](https://github.com/AdguardTeam/AdGuardHome) 的加固发行版，DNS 转发栈采用 [`LensDNS/dnsproxy`](https://github.com/LensDNS/dnsproxy)，面向负载均衡后的 **DNS-over-TCP / DoT** 等代理感知部署。

> 面向现代 TCP/TLS 接入的高性能、代理感知 DNS。

在保持与上游兼容的前提下，提供 **Proxy Protocol v2**、传输层行为增强，以及面向 LensDNS 的发行与更新机制。血统：[AdguardTeam/AdGuardHome](https://github.com/AdguardTeam/AdGuardHome)。

```text
Client → HAProxy / Nginx / LB → (PPv2) → LensDNS → 上游 DNS
```

## 核心能力

- **Proxy Protocol v2**（DNS-over-TCP / DoT）、严格语义与 `dns.trusted_proxies` — [docs/zh/ppv2.md](docs/zh/ppv2.md)。
- 在 PPv2 端到端启用时，于 L4/L7 负载均衡后恢复 **真实客户端 IP** 语义。
- **LensDNS dnsproxy** 引擎：TLS/TCP、Keep-Alive、平台允许的 TFO、TLS 会话恢复、TCP 分帧加固等 — [docs/zh/engine.md](docs/zh/engine.md)。
- **发行与更新**：GitHub Releases、`version.json`、CI 默认注入 — [docs/zh/releases.md](docs/zh/releases.md)。
- **Docker**（GHCR）与 **Release 二进制**（Actions 构建）。
- **上游 README** 嵌入于下文，含通用安装、构建与社区链接。
...
**Release 二进制**：[GitHub Releases](https://github.com/LensDNS/LensDNS/releases)。

**源码构建**：见下方嵌入的上游 README 中的 **How to build**（Go/Node 工具链）。

## 文档

- [LensDNS 文档索引（中文）](docs/zh/README.md)
- [PPv2 与可信代理](docs/zh/ppv2.md)
- [发行与 version.json](docs/zh/releases.md)
- [DNS 引擎（dnsproxy）](docs/zh/engine.md)
- 核心库：[github.com/LensDNS/dnsproxy](https://github.com/LensDNS/dnsproxy)

## 引擎

DNS 转发依赖 [`github.com/LensDNS/dnsproxy`](https://github.com/LensDNS/dnsproxy)。传输层细节、开关与讨论见该仓库。
