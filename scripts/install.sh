...
#
# See https://github.com/LensDNS/LensDNS/issues/2443.
fix_darwin() {
	if [ "$os" != 'darwin' ]; then
		return 0
	fi
...
rerun_with_root() {
	script_url='https://raw.githubusercontent.com/LensDNS/LensDNS/master/scripts/install.sh'
	readonly script_url
...
install_service() {
	...
	#
	# See https://github.com/LensDNS/LensDNS/issues/2542.
	if [ "$cpu" = 'armv7' ]; then
		cpu='armv5'
		reinstall='1'
